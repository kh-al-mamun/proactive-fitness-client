            ...
            .then(result => {
                if (result.user) {
                    updateProfile(result.user, { displayName: name, photoURL: photoUrl })
                        .then(async () => {
                            const newUser = { name, email, image: photoUrl, role: 'student', enrolled_classes: [], isBanned: false, booked_classes: [] };
                            console.log('hitting saveNewUserToDataBase func')
                            await saveNewUserToDatabase(newUser, toastId);
                            console.log('after saveNewUser')
        ...
        <div>
            <div className="lg:flex flex-row-reverse">
                <div className="lg:w-1/2 xl:max-w-screen-sm">
                    ...
                    <div className="mt-10 px-12 sm:px-24 md:px-48 lg:px-12 lg:mt-6 xl:px-24 xl:max-w-2xl">
                        ...
                        <div className="mt-12">
                            <form onSubmit={handleSubmit(handleSignUp)}>
                                <div>
                                    ...
                                    <input className="blocky-input" type="text" placeholder="Enter your full name" {...register("name", { required: true })}/>
                                </div>
                                <div className="mt-8">
                                ...
                                <div className="mt-8">
                                    <div className="text-sm font-bold text-gray-700 tracking-wide">Photo Url</div>
                                    <input className="blocky-input" type="text" placeholder="enter direct link of your photo" {...register("photoUrl", { required: true })}/>
                                </div>
                                <div className="mt-8">
                                    ...
                                    <input className="blocky-input" type="password" placeholder="Enter your password" {...register("password", { required: true, minLength: 6, pattern: /^(?=.*?[A-Z])(?=.*?[#?!@$%^&*-]).{6,}$/ })}/>
                                    {errors.password?.type === 'minLength' && <p role="alert" className="text-xs">Password must be at least 6 characters long.</p>}
                                    {errors.password?.type === 'pattern' && <p role="alert" className="text-xs">Password must contain one uppercase later and one special character.</p>}
                                </div>
                                <div className="mt-8">
                                    <div className="flex justify-between items-center">
                                        <div className="text-sm font-bold text-gray-700 tracking-wide">
                                    ...
                                    <input className="blocky-input" type="password" placeholder="Enter your password" {...register("confirmPassword", { required: true })}/>
                                </div>
                                <div className="mt-10">
                            ...
                            <div className="mt-4 text-sm font-display font-semibold text-gray-700 text-center">
                                Already have an account ? <Link to={'/sign-in'} className="cursor-pointer text-indigo-600 hover:text-indigo-800">Login</Link>
                            </div>
                        </div>
                    </div>
                </div>
